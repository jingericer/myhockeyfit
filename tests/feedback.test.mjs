...
function setup(){
 ...
 const inbox=new FeedbackInbox(state);return {env:{ADMIN_LIMITER:{limit:async()=>({success:true})},FEEDBACK_INBOX:{idFromName:k=>k,get:()=>({fetch:(url,init)=>inbox.fetch(new Request(url,init))})}},db};
}
...
test('anonymous feedback, idempotency, validation, private export and abuse limit',async()=>{
 ...
 assert.equal((await worker.fetch(new Request('https://myhockeyfit.com/api/feedback'),env)).status,503);
 ...
 assert.equal((await worker.fetch(new Request('https://myhockeyfit.com/api/feedback',{headers:{Authorization:'Bearer wrong'}}),{...env,FEEDBACK_ADMIN_TOKEN:'test-secret'})).status,403);
 assert.equal((await worker.fetch(new Request('https://myhockeyfit.com/api/feedback'),{...env,FEEDBACK_ADMIN_TOKEN:'test-secret',ADMIN_LIMITER:{limit:async()=>({success:false})}})).status,429);
